...
module.exports = function (gulp, plugins) {
	...
	function _taskFunction (gulp, plugins) {
		...
		var filePath = cmds['' + utils.MIND_COMMAND_PREFIX + 'src'];
		...
		var shouldFilter = cmds['' + utils.MIND_COMMAND_PREFIX + 'src'] !== 'false';
		...
		let currentPath = '';
		let pathBrokenDown = (writeDest.indexOf('/') < 0) ? writeDest.split('\\') : writeDest.split('/');
		for (let i = 0, iLength = pathBrokenDown.length; i < iLength; i++) {
			currentPath = currentPath + pathBrokenDown[i] + '/';
			if (!libFs.existsSync(currentPath)) { libFs.mkdirSync(currentPath); }
		}

		if (shouldReturnJson) {
			outData = JSON.stringify(levelObj, null, 2);
		...
		if (shouldJsonSchema) {
			...
			var outFileData = (typeof jsonCreated === 'string') ? jsonCreated : JSON.stringify(jsonCreated, null, 2);
	...
	function _formatToXml (inFile) {
		...
		if (typeof inFile === 'string') {
			...
			xmlDoc = (resultParser.nodeType === Node.DOCUMENT_NODE) ? resultParser.childNodes[0] : resultParser;
		} else if (!inFile) {
	...
	function _recursiveXmlToJson (xmlDoc) {
		...
		if (xmlDoc.nodeType === Node.ELEMENT_NODE) {
			let attrLength = (xmlDoc.attributes && xmlDoc.attributes.length > 0) ? xmlDoc.attributes.length : 0;
			let iAttr = 0;
			let attr = null;
			let attrName = null;
			let attrVal = null;
		...
		if (xmlDoc.hasChildNodes() && (shouldIgnoreTag === false)) {
			...
			let iChildNode = 0;
			for (iChildNode = 0; iChildNode < childrenLength; iChildNode++) {
