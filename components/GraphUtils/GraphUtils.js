...
export function exportGraphicToSprite (graphic) {
	const TEXTURE_RES = 2;
	let textureId = 'graph' + JSON.stringify(graphic.graphicsData);
	let texture = MindTextureManager.getTexture(textureId);

	if (!texture) {
		texture = MindTextureManager.generateTextureFromDisplayObject(graphic, undefined, undefined, TEXTURE_RES, true);
		MindTextureManager.saveTexture(textureId, texture);
	}

	let resultSprite = new MindPixiSprite(texture);
	return resultSprite;
}
