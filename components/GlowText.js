...
export class GlowText extends MindPixiContainer {
	constructor (text = undefined, style = {}) {
        ...
        /**
         * Defines the way signs will be positioned.
         * false: signLabels are positioned based on LEADING_CHAR_Y_OFFSETS
         * true: signLabels use superscript text style
         * @type {boolean}
         */
	...
	_toSuperscriptStyle (originalFontStyle) {
		const font = Object.assign({}, originalFontStyle);
		font.fontFamily = 'Nunito_superscript';
		return font;
	}
	...
	updateMainStyle () {
		...
		if (this.mainTextSignLabel) {
			this.mainTextSignLabel.style = this.useSuperscript ? this._toSuperscriptStyle(this.mainStyle) : this.mainStyle;
		}
	}
	...
	updateBlueStyle () {
		...
		if (this.blueTextSignLabel) {
			this.blueTextSignLabel.style = this.useSuperscript ? this._toSuperscriptStyle(this.blueStyle) : this.blueStyle;
		}
	}
	...
	updateRedStyle () {
		...
		if (this.redTextSignLabel) {
			this.redTextSignLabel.style = this.useSuperscript ? this._toSuperscriptStyle(this.redStyle) : this.redStyle;
		}
	}
	...
	updateWhiteStyle () {
		...
		if (this.whiteTextSignLabel) {
			this.whiteTextSignLabel.style = this.useSuperscript ? this._toSuperscriptStyle(this.whiteStyle) : this.whiteStyle;
		}
	}
	...
	static extractStyleStatic (theme, styleLocation) {
        ...
        // some assertions
		if (vStyleLocation.length === 0) throw new Error(`Font styleID ${styleLocation} not found in theme file!`);
		...
		if (vStyleLocation.length === 1) { // font style is defined directly at theme file
			...
			for (let i = 1; i < vStyleLocation.length; i++) {
				...
				if (!lastStyleObj.hasOwnProperty(childStyleName)) {
					throw new Error(`Font styleID ${styleLocation} not found in theme file!, ${childStyleName} is not defined as a property object`);
				}
